...
class Documentos extends BaseController
{
    ...
    public function index()
    {
        ...
        $builder = $db->table('alunos a')
            ->select('a.id_aluno, a.nome, a.data_nascimento, a.genero, a.bi, a.naturalidade,
    ...
    public function boletim(int $alunoId)
    {
        ...
        if (!$aluno) return redirect()->to(base_url('documentos'))->with('erro', 'Aluno não encontrado.');
        ...
        foreach ($notas as &$nd) {
            $mts = array_filter(array_column($nd['trimestres'], 'mt'), function($val) { 
                return $val !== null; 
            });
            $nd['media_anual'] = count($mts) ? round(array_sum($mts) / count($mts), 2) : null;
            $nd['situacao']    = ($nd['media_anual'] !== null) ? ($nd['media_anual'] >= 10 ? 'Aprovado' : 'Reprovado') : 'Pendente';
    ...
    public function declaracaoComNotas(int $alunoId)
    {
        ...
        $notas = $db->query("
            ...
            JOIN disciplinas d ON d.id_disciplina = n.id_disciplina
            JOIN turmas t      ON t.id_turma = n.id_turma
            WHERE n.id_aluno = ? AND t.ano_letivo = ?
            GROUP BY n.id_disciplina
            ORDER BY d.nome_disciplina ASC
        ", [$alunoId, $ano])->getResultArray();

        $mediaGeral = count($notas) ? round(array_sum(array_column($notas, 'media_final')) / count($notas), 2) : 0;
        $escola = $this->_getEscola();
    ...
    public function lancarNotas(int $alunoId)
    {
        ...
        $aluno = $this->_getAluno($db, $alunoId, $ano);
        if (!$aluno) return redirect()->to(base_url('documentos'))->with('erro', 'Aluno não matriculado.');

        $turmaId = $this->request->getGet('turma_id') ?? $aluno['id_turma'] ?? null;
        $disciplinas = $db->table('disciplinas')->orderBy('nome_disciplina', 'ASC')->get()->getResultArray();

        $notasExist = [];
        if ($turmaId) {
            $raw = $db->table('notas')->where('id_aluno', $alunoId)->where('id_turma', $turmaId)->get()->getResultArray();
            foreach ($raw as $r) {
        ...
        echo view('templates/header');
        echo view('templates/sidebar');
        echo view('documentos/lancar_notas', compact('aluno', 'disciplinas', 'notasExist', 'ano', 'turmaId', 'professorId'));
        echo view('templates/footer');
    }
    ...
    public function salvarNotas()
    {
        ...
        foreach ($notasPost as $discId => $trimestres) {
            foreach ($trimestres as $tri => $c) {
                $mac = ($c['mac'] !== '') ? (float)$c['mac'] : null;
                $npp = ($c['npp'] !== '') ? (float)$c['npp'] : null;
                $npt = ($c['npt'] !== '') ? (float)$c['npt'] : null;
                ...
                $dados = [
                    ...
                    'npp'           => $npp,
                    'npt'           => $npt,
                    'falta'         => ($c['falta'] !== '') ? (int)$c['falta'] : 0,
                    'observacao'    => trim($c['observacao'] ?? '') ?: null,
                ];

                $where = ['id_aluno' => $alunoId, 'id_turma' => $turmaId, 'id_disciplina' => (int)$discId, 'trimestre' => (int)$tri];
                $existe = $db->table('notas')->where($where)->countAllResults();

                if ($existe) {
                    unset($dados['id_aluno'], $dados['id_turma'], $dados['id_disciplina'], $dados['trimestre']);
                    $db->table('notas')->where($where)->update($dados);
                } else {
                    $db->table('notas')->insert($dados);
                }
            }
        }
        return redirect()->to(base_url("documentos/lancar-notas/{$alunoId}?ano={$anoLetivo}&turma_id={$turmaId}"))->with('sucesso', 'Notas guardadas com sucesso!');
    }

    // ─────────────────────────────────────────────────────────
    // HELPERS (Privados) - ATUALIZADOS PARA PEGAR BI E NATURALIDADE
    // ─────────────────────────────────────────────────────────
    private function _getAluno($db, int $id, string $ano): ?array
    {
        // ADICIONEI bi, naturalidade e provincia_natural no SELECT abaixo
        $select = 'a.id_aluno, a.nome, a.data_nascimento, a.genero, a.bi, a.naturalidade, a.provincia_natural,  a.telefone, a.email, a.nome_responsavel, t.id_turma, t.nome_turma,  t.classe, t.periodo, t.ano_letivo, m.status AS status_matricula, m.data_inscricao';
    ...
    {
        return [
            'nome'          => 'Instituto Politécnico Industrial "17 de Dezembro"',
            'nome_completo' => 'Instituto Politécnico Industrial Nº 6081 "17 de Dezembro"',
            'endereco'      => 'Luanda, Angola',
            'ministerio'    => 'Ministério da Educação de Angola',
            'provincia'     => 'Luanda',
            'directora'     => 'Joana Francisco da Rocha Francisco',
        ];
    }
}